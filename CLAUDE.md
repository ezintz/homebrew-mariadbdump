...
- `.github/workflows/bottles.yml` — builds bottles (arm64_sonoma, x86_64_linux) on push to `main` touching `Formula/**.rb`, or via manual dispatch. No x86_64 macOS bottle: GitHub retired Intel-based macOS hosted runners. Relies on `Homebrew/actions/setup-homebrew` auto-tapping this repo (name matches `owner/homebrew-*`) rather than checking it out manually — that action wipes and replaces `$GITHUB_WORKSPACE` itself, so don't reintroduce a separate `actions/checkout` step in these jobs without accounting for that. `brew bottle` is called with `--no-rebuild`: without it, brew compares the formula against `origin/HEAD` (which `setup-homebrew` always checks out exactly) and always bumps the rebuild number, producing `*.bottle.1.tar.gz` filenames the workflow's globs don't match. Merges bottle hashes into the formula, commits `[ci]`, pushes with an explicit `git push origin HEAD:main` (plain `git push` fails — `setup-homebrew`'s checkout never sets an upstream tracking branch), publishes bottle tarballs to the rolling `bottles` GitHub Release, then generates SLSA3 build provenance.
